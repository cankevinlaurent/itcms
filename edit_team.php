...
<head>
  <title>编辑 - 安全团队</title>
  <meta name = "keywords" content = "security team, edit" />
  <meta name = "description" content = "编辑安全团队" />
	...
	<div id = "bodycontent">
	  <div>
	    <form action = "" method = "get">
	      <table>
		    <tr><th>TEAM_ID</th><th>NAME</th><th>CELLPHONE</th><th>E-MAIL</th><th>DESCRIPTION</th><th>ACTION</th></tr>
			<tr>
			  <?php
				$index_id = trim($_GET["id"]);
			    $new_name = trim($_GET["new_name"]);
				$new_mphone = trim($_GET["new_mphone"]);
				$new_email = trim($_GET["new_email"]);
			    $new_desc = trim($_GET["new_desc"]);
			    if($index_id && $new_name && $new_email){
			      $result = $conn->exec("UPDATE team SET name = '".$new_name."', mphone = '".$new_mphone."', email = '".$new_email."', desc = '".$new_desc."' WHERE id = ".$index_id);
			      if($conn->changes())echo "<p>UPDATE SUCCESSFULLY</p>";
				  else echo "<p>FAILED</p>";
			    }
				$result = $conn->query("SELECT * FROM team WHERE id = ".$index_id);
			    $row = $result->fetchArray();
              ?>
			  <td><?php echo $row["id"]; ?><input type = "hidden" name = "id" value = "<?php echo $row["id"]; ?>" /></td>
			  <td><input type = "text" class = "white" name = "new_name" value = "<?php echo $row["name"]; ?>" /></td>
			  <td><input type = "text" class = "white" name = "new_mphone" value = "<?php echo $row["mphone"]; ?>" /></td>
			  <td><input type = "text" class = "white" name = "new_email" value = "<?php echo $row["email"]; ?>" /></td>
