...
<head>
  <title>编辑 - 运营室</title>
  <meta name = "keywords" content = "operation department, edit" />
  <meta name = "description" content = "编辑运营室" />
	...
	<div id = "bodycontent">
	  <div>
	    <form action = "" method = "get">
	      <table>
		    <tr><th>DEPT_ID</th><th>NAME</th><th>DESCRIPTION</th><th>ACTION</th></tr>
			<tr>
			  <?php
			    ...
			    if($index_id && $new_name){
			      $result = $conn->exec("UPDATE dept SET name = '".$new_name."', desc = '".$new_desc."' WHERE id = ".$index_id);
			      if($conn->changes())echo "<p>UPDATE SUCCESSFULLY</p>";
				  else echo "<p>FAILED</p>";
			    }
		        $result = $conn->query("SELECT * FROM dept WHERE id = ".$index_id);
