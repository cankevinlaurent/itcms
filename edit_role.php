...
<head>
  <title>编辑 - 安全角色</title>
  <meta name = "keywords" content = "security role, edit" />
  <meta name = "description" content = "编辑安全角色" />
	...
	<div id = "bodycontent">
	  <div>
	    <form action = "" method = "get">
	      <table>
		    <tr><th>ROLE_ID</th><th>NAME</th><th>DESCRIPTION</th><th>ACTION</th></tr>
			<tr>
			  <?php
			    ...
			    if($index_id && $new_name){
			      $result = $conn->exec("UPDATE role SET name = '".$new_name."', desc = '".$new_desc."' WHERE id = ".$index_id);
			      if($conn->changes())echo "<p>UPDATE SUCCESSFULLY</p>";
				  else echo "<p>FAILED</p>";
			    }
		        $result = $conn->query("SELECT * FROM role WHERE id = ".$index_id);
