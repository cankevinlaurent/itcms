...
<head>
  <title>编辑 - 机房</title>
  <meta name = "keywords" content = "information data center, edit" />
  <meta name = "description" content = "编辑机房" />
	...
	<div id = "bodycontent">
	  <div>
	    <form action = "" method = "get">
	      <table>
		    <tr><th>IDC_ID</th><th>NAME</th><th>DESCRIPTION</th><th>ACTION</th></tr>
			<tr>
			  <?php
			    ...
			    if($index_id && $new_name){
			      $result = $conn->exec("UPDATE idc SET name = '".$new_name."', desc = '".$new_desc."' WHERE id = ".$index_id);
			      if($conn->changes())echo "<p>UPDATE SUCCESSFULLY</p>";
				  else echo "<p>FAILED</p>";
			    }
		        $result = $conn->query("SELECT * FROM idc WHERE id = ".$index_id);
