...
<head>
  <title>编辑 - IP类型</title>
  <meta name = "keywords" content = "ip type, edit" />
  <meta name = "description" content = "编辑IP类型" />
	...
	<div id = "bodycontent">
	  <div>
	    <form action = "" method = "get">
	      <table>
		    <tr><th>IPTYPE_ID</th><th>NAME</th><th>DESCRIPTION</th><th>ACTION</th></tr>
			<tr>
			  <?php
			    ...
			    if($index_id && $new_name){
			      $result = $conn->exec("UPDATE iptype SET name = '".$new_name."', desc = '".$new_desc."' WHERE id = ".$index_id);
			      if($conn->changes())echo "<p>UPDATE SUCCESSFULLY</p>";
				  else echo "<p>FAILED</p>";
			    }
		        $result = $conn->query("SELECT * FROM iptype WHERE id = ".$index_id);
