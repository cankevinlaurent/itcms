...
<head>
  <title>编辑 - IT系统</title>
  <meta name = "keywords" content = "IT system, edit" />
  <meta name = "description" content = "编辑IT系统" />
	...
	<div id = "bodycontent">
	  <div>
	    <form action = "" method = "get">
	      <table>
		    <tr><th>ITSYS_ID</th><th>NAME</th><th>IMPORTANCE</th><th>DESCRIPTION</th><th>ACTION</th></tr>
			<tr>
			  <?php
		        $index_id = trim($_GET["id"]);
			    $new_name = trim($_GET["new_name"]);
				$new_weight = trim($_GET["new_weight"]);
			    $new_desc = trim($_GET["new_desc"]);
			    if($index_id && $new_name){
			      $result = $conn->exec("UPDATE itsys SET name = '".$new_name."', weight = ".$new_weight.", desc = '".$new_desc."' WHERE id = ".$index_id);
			      if($conn->changes())echo "<p>UPDATE SUCCESSFULLY</p>";
				  else echo "<p>FAILED</p>";
			    }
		        $result = $conn->query("SELECT * FROM itsys WHERE id = ".$index_id);
			    $row = $result->fetchArray();
              ?>
			  <td><?php echo $row["id"]; ?><input type = "hidden" name = "id" value = "<?php echo $row["id"]; ?>" /></td>
			  <td><input type = "text" class = "white" name = "new_name" value = "<?php echo $row["name"]; ?>" /></td>
			  <td><input type = "text" class = "white" name = "new_weight" value = "<?php echo $row["weight"]; ?>" /></td>
